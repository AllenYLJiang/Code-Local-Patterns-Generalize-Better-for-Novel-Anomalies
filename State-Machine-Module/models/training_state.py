...
class Trainer:
        ...
        for epoch in range(start_epoch, num_epochs):
            ...
            for itern, data_arr in enumerate(pbar):
                try:
                    ...
                    samp = samp.permute(0,2,1,3).reshape(-1, seq_len, 36).float()
                    ...
                    losses = self.lossf(pred, samp[:,self.input_frame:seq_len,:].reshape(-1, (seq_len-self.input_frame )* 36))
                    losses.backward()
    ...
    def normalize_data_transformed(self, data_transformed):
        ...
        normalize_pose_func_args = {'headless': False, 'scale': 0, 'scale_proportional': 1, 'seg_len': 24,
